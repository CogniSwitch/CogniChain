...
class CogniswitchStoreChain(Chain):
    ...
    def _validate_inputs(self, inputs: Dict[str, Any]) -> None:
        ...
        #     key: None for key in self.input_keys if key in self.input_keys
        # }
        # complete_inputs = {**default_input_values, **inputs}
        # if not complete_inputs["cs_token"]:
        #     raise ValueError("Missing cs_token")
        # if not complete_inputs["url"] and not complete_inputs["file"]:
        #     raise ValueError("Missing data")
        # missing_keys = set(self.input_keys).difference(complete_inputs)
        required_keys = {
            self.cs_token,
            self.OAI_token,
            self.file
